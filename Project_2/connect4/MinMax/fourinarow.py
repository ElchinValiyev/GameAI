...
def get_input():
    ...
    button = Button(master, text="OK",
                    command=ok)  # Button when clicked calls the ok() function which returns user seletion
    button.pack()
    mainloop()
    return seletion
...
def run_game(agent_1, agent_2, depth):
    ...
    while True:  # main game loop
        if turn == HUMAN:
            ...
            status = makeMove(mainBoard, RED,
                              column)  # make a move and return the row number and status of the playing board
